...
export const TagsTrigger = ({
    ...
    <PopoverTrigger asChild>
        <Button
            className={cn('h-auto w-full justify-between p-2 bg-input/30 aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive', className)}
...
export const TagsValue = ({
    ...
    return (
        <Badge className={cn('flex items-center gap-2 py-0.5', className)} {...(props as any)}>
            {children}
            {onRemove && (
                ...
                >
                    <XIcon size={10} className="w-[12px]! h-[12px]!" />
                </div>
            )}
        </Badge>
    );
...
export default function SelectionTags({ tags, values, onSelect, onRemove, onCreateNewTag, placeholder, invalid }: { tags: TagsOption[], values: string[], onSelect: (value: string) => void, onRemove: (value: string) => void, onCreateNewTag: (value: string) => void, placeholder?: string, invalid?: boolean }) {
    ...
    return (
        <Tags>
            <TagsTrigger placeholder={placeholder} aria-invalid={invalid}>
                {values.map((tag) => (
            ...
            <TagsContent>
                <TagsInput placeholder={placeholder} value={newTag} onValueChange={setNewTag} />
                <TagsList>
                    <TagsEmpty >
                        <button
